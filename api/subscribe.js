// Função serverless (Vercel) — recebe o lead da landing e:
//   1. GRAVA NO SUPABASE  (sistema de registro — CRÍTICO; se falhar, aborta e avisa)
//      - upsert em "contacts"   (1 linha por e-mail; atualiza nome/whatsapp se já existia)
//      - insert em "lead_events" (1 linha por opt-in; rastreia produto + UTMs)
...
export default async function handler(req, res) {
  ...
  const event = {
    source:       body.source       || 'desafio-7-dias',
  ...
  let brevo = 'skipped';
  try {
    brevo = await addToBrevo(contact);
  } catch (err) {
